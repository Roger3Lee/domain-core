...
function AddChecksums($filePath) {
    ...
    Write-Host "    MD5/SHA1: $($filePath | Split-Path -Leaf).md5 + .sha1"
}

foreach ($mod in $modules) {
    $destDir = Join-Path $bundleDir "io\github\roger3lee\$($mod.artifactId)\2.0.0"
    ...
    if (Test-Path $pomFile) {
        $destPom = Join-Path $destDir "$($mod.artifactId)-2.0.0.pom"
        Copy-Item $pomFile $destPom -Force
        Write-Host "  POM: $($mod.artifactId)-2.0.0.pom"
        AddChecksums $destPom
    }
    $pomAsc = Join-Path $pubDir "pom-default.xml.asc"
    if (Test-Path $pomAsc) {
        $destPomAsc = Join-Path $destDir "$($mod.artifactId)-2.0.0.pom.asc"
        Copy-Item $pomAsc $destPomAsc -Force
        Write-Host "  POM.ASC: $($mod.artifactId)-2.0.0.pom.asc"
