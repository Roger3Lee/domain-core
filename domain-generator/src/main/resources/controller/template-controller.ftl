...
public class ${controllerClassName} {
     ...
     * @return ${dtoClassName} 领域数据传输对象
     */
    @PostMapping("/{key}")
    public ${dtoClassName} find(
            @PathVariable("key") ${source.mainTable.keyType} key
    <#if (source.relatedTable?size>0)>
            ,@RequestBody(required = false) ${dtoClassName}.LoadFlag loadFlag
    </#if>
    ){
        ${NameUtils.getName(source.name)}FindDomain request = ${NameUtils.getName(source.name)}FindDomain.builder()
                .key(key)
    <#if (source.relatedTable?size>0)>
                .loadFlag(loadFlag != null ? loadFlag : new ${dtoClassName}.LoadFlag())
    </#if>
                .build();
     ...
     * @return ${source.mainTable.keyType} 新增记录的主键标识
     */
    @PutMapping()
     ...
     */
    @DeleteMapping("/{key}")
    public Boolean delete(@PathVariable("key") ${source.mainTable.keyType} key){
