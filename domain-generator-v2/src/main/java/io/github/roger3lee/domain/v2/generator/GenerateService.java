...
public class GenerateService {
     ...
     * - applications/, controllers/    : only on first run (skip if already exists)
     *
    ...
    public void generateAll(String outputPath, String basePackage,
        ...
        // 3. Generate application layer + controllers — only on first run (skip if already exists)
        boolean firstRun = !isFirstDomainGenerated(packagePath + "/domains", domainList);
        if (firstRun) {
    ...
    /**
     * Check whether the domain layer has already been generated by looking for
     * the first domain's DTO file.
     */
    private boolean isFirstDomainGenerated(String domainPath, List<DomainMetaInfo> domainList) {
        if (domainList.isEmpty()) return false;
        DomainInfo firstDomain = DomainInfo.convert(domainList.get(0), tableMetaInfoMap);
        String folder = firstDomain.getFolderPath();
        return FileUtils.fileExists(domainPath + "/" + folder + "/domain",
                firstDomain.nameSuffix("Domain") + ".java");
    }
