...
/**
 * Naming utility for domain code generation.
 * Converts between SQL naming conventions (underscore_case) and Java conventions (camelCase/PascalCase).
 */
public class NameUtils {
    ...
    public static String getFieldName(String name) {
        // If the name is all uppercase (like ID, FK_ID), convert to lowercase first
        // so that ID -> id, FK_ID -> fkId instead of iD, fKId
        if (name != null && name.equals(name.toUpperCase()) && name.contains("_")) {
            return StringUtils.uncapitalize(StrUtil.toCamelCase(name.toLowerCase()));
        }
        if (name != null && name.equals(name.toUpperCase()) && !name.contains("_")) {
    ...
    public static String fieldRelatedSourceLambda(String name, String target, String field) {
        return getFieldWithSuffix(name, getFieldName(field) + "_Related" +
                StringUtils.capitalize(StrUtil.toCamelCase(StrUtil.format("{}", target))) + "_" + "SourceLambda");
    }
    ...
    public static String genGetter(String fieldName) {
        // Handle all-uppercase names: ID -> getId, FK_ID -> getFkId
        String camelName = getFieldName(fieldName);
        return StrUtil.genGetter(camelName);
    }

    public static String genListGetter(String fieldName) {
        String camelName = getFieldName(fieldName);
        return StrUtil.genGetter(camelName) + "List";
    }

    public static String genSetter(String fieldName) {
        // Handle all-uppercase names: ID -> setId, FK_ID -> setFkId
        String camelName = getFieldName(fieldName);
        return StrUtil.genSetter(camelName);
    }

    public static String genListSetter(String fieldName) {
        String camelName = getFieldName(fieldName);
        return StrUtil.genSetter(camelName) + "List";
    }
}
